...
const EditAlbum = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await axios.patch(`http://localhost:3001/api/albums/${id}`, {
        name: albumName,
        date: albumDate
  ...
  const handleDeletePhoto = async (photoId: number) => {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/photos/${photoId}`, { withCredentials: true });
      setData(prev => prev ? {
        ...prev,
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {data.photos.map(photo => (
            <div key={photo.id} className="group relative aspect-square rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
              <img 
